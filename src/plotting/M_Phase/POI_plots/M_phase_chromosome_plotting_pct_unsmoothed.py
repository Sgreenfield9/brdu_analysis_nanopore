...
import plotting.M_Phase.genome_browser.M_phase_chromosome_plotting_unsmoothed as m
...
def load_M_pct_10kb():
    ...
    """
    M_10kb_pct = os.getenv("BRDU_M_PCT")
    if not M_10kb_pct:
        raise ValueError("BRDU_M_PCT must be set in the env/.env")
    if not os.path.exists(M_10kb_pct):
        raise FileNotFoundError(f"BRDU_M_PCT not found at {M_10kb_pct}")
...
def get_output_dir():
    ...
    """
    output_dir = os.getenv("BRDU_PCT_OUTPUT_M_UNSMOOTHED")
    if not output_dir:
        output_dir = os.path.join(os.getcwd(), "output", "M_Phase_pct_10kb_unsmoothed")
...
# Getting our data from the M_phase_chromosome_plotting_unsmoothed.py
...
for _, row in poi.iterrows():
    ...
    # Window boundaries: 30kb total (15kb each side), with left-edge handling
    if center < 5000:
        left = 0
    ...
    xs_rel = xs - left
    centers, nmod_binned = m._binned_signal(xs_rel, nmod, chr_length=window_len, bin_size=display_bin_size)
    _, brdu_binned = m._binned_signal(xs_rel, brdu, chr_length=window_len, bin_size=display_bin_size)
